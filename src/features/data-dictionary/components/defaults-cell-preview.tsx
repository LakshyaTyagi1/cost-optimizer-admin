...
export type DefaultsCellPreviewProps = {
...
export function DefaultsCellPreview({
  activation = "hover",
  ariaLabel,
  content,
  label,
  ...
  const scheduleHidePreview = useCallback(() => {
    cancelScheduledClose();
    closeTimerRef.current = window.setTimeout(hidePreview, 120);
  }, [cancelScheduledClose, hidePreview]);
  ...
  return (
    <>
      <button
        ...
        aria-label={ariaLabel || `View ${label.toLowerCase()} for ${title}`}
        onFocus={opensOnDoubleClick ? undefined : showPreview}
        ...
        onDoubleClick={opensOnDoubleClick ? showPreview : undefined}
        onMouseEnter={opensOnDoubleClick ? undefined : showPreview}
        onMouseLeave={opensOnDoubleClick ? undefined : scheduleHidePreview}
        onPointerDown={(event) => {
            ...
            >
              ...
              <div className="flex items-center gap-2 border-b border-black/[0.06] px-4 py-2.5">
                ...
                <div className="min-w-0">
                  ...
                  <p className="truncate text-xs font-bold text-[#333]">{title}</p>
                </div>
              </div>
